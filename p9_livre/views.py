...
from p9_livre.models import Ticket ,Review
from itertools import chain
from .formulaires import DemandeDeCritiquePublication, RepondreCritique, ReviewForm, UserFollowForm
from .models import Ticket , Review, UserFollows
from django.db.models import CharField, Value
from django.shortcuts import render ,redirect
from . import models , formulaires
...
def afficher_review(request):
    ...
    return render(request, 'Home.html', context={'posts': posts , 'publication': DemandeDeCritiquePublication()})
...
def afficher_flux(request):
    mes_tickets = Ticket.objects \
        .filter(user=request.user.id) \
        .all() 
    mes_critiques = Review.objects.filter(user=request.user.id).all()

    return render(request,"posts.html", context={"tickets":mes_tickets,"reviews":mes_critiques}) 
...
def publication_ticket(request):
    ...
    posts = Ticket.objects.filter(user=request.user.id).all() 
    # returns queryset of tickets
...
def repondre_ticket(request,ticket_id):
    ...
    return render(request, 'fenêtre_repondre.html' , context={'ticket': ticket , 'reponse':publication })

@login_required
def afficher_abonnement(request):
    user = request.user.following
...
class Subscriptions(View):
    ...
    def get(self, request):
        ...
        return render(request, 'abonnement.html', context={'user_follows_form': user_follows_form,
    ...
    def post(self, request):
        ...
        if not get_user_model().objects.filter(username=followed_user_username).exists():
            error_message = 'unknown_user'
        elif followed_user_username == actual_user.username:
            error_message = 'do_not_follow_yourself'
        else:
            followed_user_id = get_user_model().objects.get(username=followed_user_username).id
